...
export default function JoinScreen({ roomCode, onJoin, onBack }: Props) {
  ...
  return (
     ...
     * This screen can be taller than the viewport on small phones
     * (especially with the server URL field expanded + keyboard open).
     * We let it scroll naturally instead of forcing fixed height.
     */
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="mb-5 flex items-center gap-3 bg-indigo-500/10 border border-indigo-500/40 rounded-2xl px-5 py-4">
          ...
          <div>
            ...
            <p className="text-indigo-400/80 text-xs mt-0.5">
              Room code:{' '}
              <span className="font-mono font-black text-indigo-300 text-sm tracking-widest">
                {roomCode}
              </span>
            </p>
          </div>
        ...
        <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 backdrop-blur-sm shadow-2xl">
          ...
          <p className="text-slate-400 text-sm mb-5">
            You'll join the private chat room instantly.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 flex items-center justify-between">
              ...
              <span className="text-white font-mono font-black text-xl tracking-[0.25em]">
                {roomCode}
              </span>
            </div>
            ...
            <button
              type="submit"
              disabled={!name.trim()}
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 hover:-translate-y-0.5 active:translate-y-0 text-base"
            >
        ...
        {/* ✅ FIX: Back button always reachable — has enough bottom padding */}
        <button
          onClick={handleBack}
